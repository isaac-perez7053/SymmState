...
def load_flexo_data(filename):
    ...
    with open(filename, 'r') as file:
        ...
        amplitudes = ast.literal_eval(content.split('List of Amplitudes:')[1].split('\n')[0].strip())
        energies = ast.literal_eval(content.split('List of Energies:')[1].split('\n')[0].strip())
        flexo_amps = ast.literal_eval(content.split('List of Flexo Amplitudes:')[1].split('\n')[0].strip())
...
def plot_energy(amplitudes, energies, ax=None):
    ...
    ax.set(xlabel='Amplitude (bohr)', ylabel='Energy (Ha)',
...
def plot_flexo_components(flexo_amps, flexo_tensors, ax=None):
    ...
    for comp_idx in range(54):
        ...
        ax.plot(flexo_amps, components, alpha=0.5, linewidth=0.8,
                label=get_component_label(comp_idx))

    ax.set(xlabel='Amplitude (bohr)', ylabel='Component Value (nC/m)',
...
def plot_varying_components(flexo_amps, flexo_tensors, threshold='auto', figsize=(15, 10)):
    ...
    for idx, comp_idx in enumerate(varying_indices):
        ...
        ax.set_title(f'{get_component_label(comp_idx)}\n(σ={variations[comp_idx]:.2e})', fontsize=10)
        ax.grid(True, alpha=0.3)
