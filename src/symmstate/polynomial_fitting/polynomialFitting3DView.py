...
def parse_term(term: str) -> Tuple[int, int]:
    ...
    x_exp = int(x_match.group(1)) if x_match and x_match.group(1) else 1 if 'x' in term else 0
    y_exp = int(y_match.group(1)) if y_match and y_match.group(1) else 1 if 'y' in term else 0
...
def validate_input_terms(terms: List[str]) -> None:
    ...
    for term in terms:
        if not valid_pattern.match(term):
            raise ValueError(f"Invalid term format: {term}. Valid examples: 'x^2', 'xy^3', 'const'")

def get_initial_guess(x: np.ndarray, y: np.ndarray, z: np.ndarray, terms: List[str]) -> np.ndarray:
...
def generate_latex_equation(popt: np.ndarray, terms: List[str]) -> str:
    ...
    for coef, term in zip(popt, terms):
        if term == 'const':
            ...
            parts = []
            if 'x' in term:
                x_part = 'x' + (f"^{{{term.count('x')}}}" if term.count('x')>1 else '')
                parts.append(x_part)
            if 'y' in term:
                y_part = 'y' + (f"^{{{term.count('y')}}}" if term.count('y')>1 else '')
                parts.append(y_part)
            equation += f"{coef:.2f}" + "".join(parts) + " + "
    return equation.rstrip(" + ").replace('^1', '').replace('x^0', '').replace('y^0', '')
...
def parse_args() -> Tuple[str, List[str]]:
    ...
    if len(sys.argv) < 3 or '-h' in sys.argv:
        print("""Polynomial Surface Fitter
...
def plot_results(
    ...
    ax.set_title(f'Surface Fit - Step {step}\n{generate_latex_equation(popt, terms)}', fontsize=12)
...
def main():
    ...
    popt, mse, r2, _ = fit_and_evaluate(x_scaled, y_scaled, z_scaled, final_func, initial_guess)
    ...
    plot_results(x_scaled, y_scaled, z_scaled, final_func, popt, 
                selected_terms, "Final", "final_fit.png")
