...
class SmodesProcessor(FlpzCore):
    ...
    def _loop_modes(self):
        ...
        self.slurm_obj.running_jobs.append("dist_0.abo")
        ...
        for i in range(self.num_sam):
            ...
            self.abinit_file.change_coordinates(np.array(original_coords).copy(), coords_are_cartesian=False)
            self.slurm_obj.running_jobs.append(f"dist_{j}.abo")
    ...
    def _perform_calculations(self, stabilize=False):
        ...
        self._logger.info(f"Condition number of the force constants matrix: {cond_number}")
        if cond_number > 1e5:
        ...
        self._logger.info(f"DEBUG: dynevecs_sam:\n{dynevecs_sam}")
        self._logger.info(f"DEBUG: dynevals:\n{dynevals}")
        ...
        self._logger.info(f"DEBUG: freq_thz:\n{freq_thz}")
        ...
        self._logger.info(f"DEBUG: Reduced mass vector:\n{self.red_mass}")
        self._logger.info("Computation completed. Results stored in object attributes.")
    ...
    def unstable_phonons(self):
        ...
        if unstable is False:
            ...
            self._logger.info(f"DEBUG: Normalized unstable phonons:\n{unstable_normalized}")
